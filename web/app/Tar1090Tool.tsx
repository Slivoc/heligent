...
type Candidate = {address:string;hours:number;days:number;first_day:string;last_day:string;registrations:string[];types:string[];identity_sources:string[];category:string;group:string;conflicts:string[];reviewable:boolean;applied?:boolean;claim:null|{registration:string|null;type_code:string|null;description:string|null;flags:string}};
type Preview = {preview_id:string;snapshot_id:number;metadata:TarMetadata;created_at:string;from:string;to:string;region:string;gap:string;processed_days:number;expected_days:number;total:number;matched:number;reviewable:number;counts:Record<string,number>;rows:Candidate[];filtered_count:number;offset:number;has_more:boolean};
...
export function Tar1090Tool({canEdit,metadata,latestPreview,onRefresh}:{canEdit:boolean;metadata?:TarMetadata;latestPreview?:string;onRefresh:()=>void}) {
  ...
  const [filters,setFilters]=useState({from:'',to:'',region:'EU',gap:'TAIL'});
  ...
  return <section className="tools-card tar1090-tool" aria-label="tar1090 bulk source tool">
    <div className="source-overview"><div><p className="eyebrow">Bulk aircraft reference</p><h3>Find the helicopter identities hidden in your gaps.</h3><p>Download the reference, compare observed hexes, then review the useful matches.</p></div><button disabled={!canEdit||!!busy} onClick={()=>void refresh()}>{metadata?'Refresh tar1090 snapshot':'Download tar1090 snapshot'}</button></div>
    <p className="source-caption">The download retains source files for comparison. Each assignment is reviewed separately. Refreshes run on demand and can take about a minute.</p>
    ...
    {preview&&<>
      ...
      <p>{preview.processed_days}/{preview.expected_days} dates processed · {preview.total.toLocaleString()} hexes with gaps · {preview.matched.toLocaleString()} source matches · {preview.reviewable.toLocaleString()} candidates eligible for dated review at comparison time.</p>
