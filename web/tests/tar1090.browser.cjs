...
(async()=>{
  ...
  try {
    ...
    await page.getByRole('button',{name:'Review dates for F00001',exact:true}).waitFor();
    ...
    assert.equal(await page.getByRole('button',{name:'Review dates for F00002',exact:true}).count(),0);
    ...
    await page.getByRole('button',{name:'Review dates for F00001',exact:true}).click();
