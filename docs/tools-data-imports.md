...
The unidentified list defaults to **Helicopters + unknown types**, to avoid hiding untyped aircraft such as G-WSAS. Other choices are helicopters only, unknown only, fixed wing only, or all. Classification comes from recorded ICAO type classification, not inferred from callsigns.
